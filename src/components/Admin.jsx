    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm p-4">
      <div className="w-full max-w-lg bg-[#1A1A1A] rounded-3xl border border-white/10 p-6 max-h-[90vh] overflow-y-auto">
        ...
        {fotos.length > 0 ? (
          ...
          <div className="text-center py-8 text-white/40 mb-6">
  ...
  useEffect(() => {
    ...
    async function carregarDados() {
      ...
      if (filtroStatusMsg !== 'todos') {
        queryMensagens = queryMensagens.eq('status', filtroStatusMsg)
      }

      const { data: msgs } = await queryMensagens
      setMensagensRecebidas(msgs || [])
    }

    carregarDados()
  }, [autenticado, busca, filtroStatus, filtroCategoria, ordenacao, filtroStatusMsg])
    ...
    <div className="min-h-screen bg-gradient-to-br from-[#0a0a0a] to-[#1a1a1a] p-6">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex gap-4 mb-8 border-b border-white/10">
          ...
          <button onClick={() => setTab('mensagens')} className={`pb-4 font-semibold transition ${tab === 'mensagens' ? 'text-[#FF5C00] border-b-2 border-[#FF5C00]' : 'text-white/50 hover:text-white'}`}>
            💬 Mensagens
          </button>
        </div>
                  ...
                  <div key={prof.id} className="bg-white/5 border border-white/10 rounded-2xl overflow-hidden hover:border-[#FF5C00]/40 transition">
                    ...
                    <div className="p-4">
                      ...
                      <div className="space-y-2">
                        ...
                        <button onClick={() => setModalGerenciadorFotos(prof)} className="w-full bg-white/5 text-white py-2 rounded-lg text-xs font-semibold hover:bg-white/10 transition">📸 Fotos ({prof.profissional_imagens?.length || 0})</button>
                  ...
                  <div key={msg.id} className="bg-white/5 border border-white/10 rounded-2xl p-4 hover:border-[#FF5C00]/40 transition">
                    ...
                    <div className="flex gap-2 flex-wrap">
                      <button
                        ...
                        className="flex-1 min-w-[120px] bg-white/10 text-white py-2 rounded-lg text-xs font-semibold hover:bg-white/20 transition"
                      >
                        📋 Copiar
                      </button>
                      <a
                        ...
                        className="flex-1 min-w-[120px] bg-[#25D366] text-white py-2 rounded-lg text-xs font-semibold hover:bg-[#20BA5C] transition text-center"
                      >
                        💬 Chat
                      </a>
                      <select
      ...
      {modalGerenciadorFotos && <GerenciadorFotos prof={modalGerenciadorFotos} onFechar={() => setModalGerenciadorFotos(null)} onSalvar={() => { setProfissionais([...profissionais]) }} />}
    </div>
  )
}
